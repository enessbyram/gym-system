...
const PTAppointmentManagement = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      const res = await fetch(
        `/api/pt_appointments.php?action=get_all&pt_id=${user.id}&t=${Date.now()}`,
      );
  ...
  if (loading)
    return <div className="text-white text-center py-10">Yükleniyor...</div>;

  return (
    <div className="flex flex-col xl:flex-row gap-8 w-full mt-8">
      {/* --- SOL TARA: GELECEK RANDEVULAR --- */}
      {/* DEĞİŞİKLİK BURADA: h-125 yerine h-[500px] veya min-h-[500px] kullanıyoruz */}
      <div className="flex-1 bg-[#1e1e1e] border border-[#2e2e2e] rounded-3xl p-6 md:p-8 shadow-xl flex flex-col h-auto">
        ...
        {/* Debug için: Veri var mı diye ekrana basıyoruz (Geçici) */}
        {/* <div className="text-xs text-red-500">{JSON.stringify(upcoming)}</div> */}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-4 pr-2">
          {upcoming.length > 0 ? (
            upcoming.map((lesson) => (
              <div
                key={lesson.id}
                className="bg-[#252525] border border-[#383737] rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 transition-all hover:border-[#5b5b5b] shrink-0"
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#333] rounded-full flex items-center justify-center text-[#5b5b5b] shrink-0">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-[#888] flex items-center gap-3">
                      {/* Tarih ve saati basitçe gösterelim */}
                ...
                <button
                  ...
                  className="w-full sm:w-auto cursor-pointer bg-[#22c55e] hover:bg-[#1ea84f] text-white px-6 py-2.5 rounded-xl font-bold text-sm transition-all active:scale-95 flex items-center justify-center gap-2 shadow-lg shadow-green-900/20 shrink-0"
                >
      ...
      {/* BURADA DA AYNI DEĞİŞİKLİK: h-[500px] */}
      <div className="flex-1 bg-[#1e1e1e] border border-[#2e2e2e] rounded-3xl p-6 md:p-8 shadow-xl flex flex-col h-auto">
        <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-4 pr-2">
          {completed.length > 0 ? (
            completed.map((lesson) => (
              <div
                key={lesson.id}
                className="bg-[#1a2e22] border border-[#22c55e]/30 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shrink-0"
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-[#22c55e]/20 rounded-full flex items-center justify-center text-[#22c55e] shrink-0">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-[#22c55e]/70 flex items-center gap-3">
                      <span>
                        {lesson.date_formatted} • {lesson.time_formatted}
                      </span>
                    </div>
                  </div>
                </div>
                <div className="w-full sm:w-auto bg-[#22c55e]/20 text-[#22c55e] px-4 py-2 rounded-lg font-bold text-xs border border-[#22c55e]/30 flex items-center justify-center gap-2 cursor-default shrink-0">
