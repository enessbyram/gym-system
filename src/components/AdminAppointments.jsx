...
const AdminAppointments = () => {
  ...
  return (
    <div className="w-full bg-[#1e1e1e] border border-[#2e2e2e] rounded-3xl p-4 md:p-6 shadow-2xl flex flex-col relative animate-fade-in font-montserrat">
      
      {selectedDayInfo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-[#1e1e1e] border border-[#383737] w-11/12 max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-scale-up flex flex-col max-h-[80vh]">
            ...
            <div className="bg-[#252525] p-4 md:p-5 border-b border-[#383737] flex justify-between items-center shrink-0">
              ...
              <button onClick={() => setSelectedDayInfo(null)} className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-[#333] text-white hover:bg-red-500 transition-colors flex items-center justify-center">
            ...
            <div className="p-4 md:p-6 overflow-y-auto custom-scrollbar flex flex-col gap-3">
              {selectedDayInfo.list.length > 0 ? (
                selectedDayInfo.list.map((app) => (
                  <div key={app.id} className={`border p-3 md:p-4 rounded-xl flex items-center justify-between transition-all group ${app.type === 'salon' ? 'bg-[#009fe2]/5 border-[#009fe2]/30 hover:border-[#009fe2]' : 'bg-[#22c55e]/5 border-[#22c55e]/30 hover:border-[#22c55e]'}`}>
                    <div className="flex items-center gap-3 md:gap-4">
                      ...
                      <div>
                        <h4 className="text-white font-bold text-sm md:text-lg">{app.member}</h4>
                        <div className="text-[10px] md:text-xs text-[#888] flex flex-col">
                ...
                <div className="flex flex-col items-center justify-center py-12 text-[#5b5b5b] gap-3">
                  <FontAwesomeIcon icon={faCalendarDay} className="text-4xl md:text-5xl opacity-20" />
                  <p className="text-sm md:text-base">Bu tarihte planlanmış randevu yok.</p>
                </div>
              )}
            </div>
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-col gap-1 overflow-hidden">
                ...
                {daysAppointments.length > 3 && <div className="text-[8px] md:text-xs text-[#5b5b5b] text-center font-medium">+ {daysAppointments.length - 3}</div>}
              </div>
            </div>
          );
        })}
