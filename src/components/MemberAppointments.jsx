...
const MemberAppointments = ({ onBack }) => {
  ...
  useEffect(() => {
    // User ve ID kontrolü (Sayfa bozulmasını önler)
    if (user && user.id) {
  ...
  const formatDate = (dateStr) => {
    if (!dateStr) return "";
    // Sadece YYYY-MM-DD kısmını al (Saat varsa temizle)
  ...
  const isAppointmentPast = (dateStr, timeStr) => {
    // Tarihi YYYY-MM-DD formatına zorla
    const cleanDate = dateStr.split(" ")[0];
  ...
  const AppointmentCard = ({ app, isPast }) => (
    <div className={`w-full border rounded-2xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-all ${isPast ? "bg-[#1a1a1a] border-[#2e2e2e] opacity-75" : "bg-[#1e1e1e] border-[#383737] hover:border-[#009fe2] shadow-lg"}`}>
      <div className="flex items-center gap-6">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${isPast ? "bg-[#252525] text-[#5b5b5b]" : app.type === 'salon' ? "bg-[#009fe2]/10 text-[#009fe2]" : "bg-[#22c55e]/10 text-[#22c55e]"}`}>
          <FontAwesomeIcon icon={app.type === 'salon' ? faDumbbell : faBuilding} />
        </div>
        <div className="flex flex-col gap-1">
          <h3 className={`text-lg font-bold ${isPast ? "text-[#7a7a7a]" : "text-white"}`}>{app.displayTitle}</h3>
      ...
      <div className={`px-4 py-1.5 rounded-full text-xs font-bold border flex items-center gap-2 ${isPast ? "border-[#383737] text-[#5b5b5b]" : "border-[#22c55e] text-[#22c55e] bg-[#22c55e]/10"}`}>
  ...
  return (
    <div className="w-full max-w-7xl animate-fade-in">
      <button onClick={onBack} className="flex items-center cursor-pointer gap-2 text-[#5b5b5b] hover:text-white mb-6 transition-colors">
        ...
        <>
          <div className="mb-12">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="w-full py-10 border border-dashed border-[#383737] rounded-2xl flex flex-col items-center justify-center text-[#5b5b5b] gap-2"><FontAwesomeIcon icon={faCalendar} className="text-3xl opacity-20" /><p>Yaklaşan randevunuz bulunmamaktadır.</p></div>}
            </div>
          </div>
